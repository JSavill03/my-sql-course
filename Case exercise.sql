...
SELECT
	ps.PatientId
	, ps.Hospital
	, CASE WHEN PS.Hospital = 'Kings College' Then 'Urban' ELSE 'Rural' End AS HospitalLocation
FROM
	dbo.PatientStay ps
ORDER BY
	HospitalLocation;
...
SELECT
    ...
    ,ps.Tariff
    ,CASE 
    WHEN ps.Tariff >= 7 THEN 'High Tarriff'
    WHEN ps.Tariff >=4 AND ps.Tariff <7 THEN 'Medium Tarriff'
    ELSE 'Low Tarriff'
